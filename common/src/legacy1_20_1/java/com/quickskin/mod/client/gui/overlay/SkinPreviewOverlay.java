...
public class SkinPreviewOverlay {
    ...
    // Note: Old corner-based positioning system removed
    // Now uses hotbar center as reference point (similar to button-based positioning in other screens)
    ...
    public static void render(GuiGraphics guiGraphics, float tickDelta) {
        ...
        // OPTIONAL: Auto-hide when in menus (player is focused on menu, not HUD)
        // This saves 100% of render cost when player is in a menu
        // EXCEPTION: Keep visible when chat is open so you can see who is in-game
        if (mc.screen != null) {
            boolean isChatOpen = mc.screen instanceof net.minecraft.client.gui.screens.ChatScreen;
            if (!isChatOpen) {
                // Hide for all screens except chat
                return;
            }
            // Chat is open, continue to render
        }

        // Note: Could also hide when F3 debug screen is open if needed
        // (Minecraft API varies by version, so this is kept simple)
        ...
        // Use hotbar center as reference point (like other player previews use buttons)
        int screenWidth = mc.getWindow().getGuiScaledWidth();
        int screenHeight = mc.getWindow().getGuiScaledHeight();

        // Hotbar center reference point
        int hotbarCenterX = screenWidth / 2;
        int hotbarCenterY = screenHeight - 22; // Hotbar is typically 22 pixels from bottom

        // Calculate model position using hotbar center + offsets from config
