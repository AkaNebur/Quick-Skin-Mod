...
public class ServerNetworkHandler {
        ...
        for (ServerPlayer otherPlayer : player.server.getPlayerList().getPlayers()) {
            if (!otherPlayer.getUUID().equals(player.getUUID()) && canReceiveQuickSkin(otherPlayer)) {
                ...
                NetworkTransport.INSTANCE.sendTextureToPlayer(
                        otherPlayer, textureType, hash, imageData);
            }
        }
        ...
        for (ServerPlayer otherPlayer : player.server.getPlayerList().getPlayers()) {
            if (!otherPlayer.getUUID().equals(player.getUUID()) && canReceiveQuickSkin(otherPlayer)) {
                ...
                NetworkTransport.INSTANCE.sendAppearanceToPlayer(
                        otherPlayer, player.getUUID(), skinId, capeId, model);
            }
        }
    ...
    public static void sendAppearanceToPlayer(ServerPlayer recipient, UUID targetPlayerId) {
        ...
        if (appearance != null) {
            ...
            if (skinId != null && skinId.startsWith("local_skin:")) {
                ...
                if (skinData != null) {
                    NetworkTransport.INSTANCE.sendTextureToPlayer(recipient, "skin", hash, skinData);
                }
            }
            ...
            if (capeId != null && capeId.startsWith("local_cape:")) {
                ...
                if (capeData != null) {
                    NetworkTransport.INSTANCE.sendTextureToPlayer(recipient, "cape", hash, capeData);

                    // Also send animation metadata if available
                    String metadata = ServerAnimationCache.getInstance().getMetadata(hash);
                    if (metadata != null) {
                        NetworkTransport.INSTANCE.sendAnimationMetadataToPlayer(recipient, hash, metadata);
                    }
                }
            }
    ...
    private static void sendTextureToClient(ServerPlayer player, String textureType, String hash, byte[] textureData) {
        ...
        NetworkTransport.INSTANCE.sendTextureToPlayer(player, textureType, hash, textureData);
    }
