...
public class AnimatedTextureManager {
    ...
    private static class AnimationState {
        ...
        AnimationState(String animationId, ResourceLocation atlasLocation,
            ...
            String texId = "animated/" + animationId.replaceAll("[^a-zA-Z0-9/._-]", "_");
            this.frameTexture = new DynamicTexture(() -> "quickskin_" + texId, framePixels);

            // Register single texture with a unique name
            this.frameTextureLocation = ResourceLocation.fromNamespaceAndPath(QuickSkin.MOD_ID, texId);
        ...
        private void copyFrameTo(NativeImage target, int frameIndex) {
            ...
            for (int y = 0; y < frameHeight; y++) {
                for (int x = 0; x < frameWidth; x++) {
                    PlatformHelper.setPixel(target, x, y, PlatformHelper.getPixel(atlasPixels, x, srcY + y));
                }
            }
        }
     ...
     * With the optimized architecture, this always returns the same ResourceLocation
