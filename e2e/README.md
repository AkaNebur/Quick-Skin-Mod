...
Minecraft 1.21.7 release files and two matching runtime rows: Fabric and NeoForge. Every artifact
advertises and launches on exactly Minecraft 1.21.7; matrix validation fails if any consumer-facing
...
xvfb-run -a python e2e/orchestrator.py \
  --packaged \
  --artifact-node fabric-1.21.7 \
  --runtime-version 1.21.7 \
