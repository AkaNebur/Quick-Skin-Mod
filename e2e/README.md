...
Minecraft 1.21.2 release files and two matching runtime rows: Fabric and NeoForge. Every artifact
advertises and launches on exactly Minecraft 1.21.2; matrix validation fails if any consumer-facing
...
xvfb-run -a python e2e/orchestrator.py \
  --packaged \
  --artifact-node fabric-1.21.2 \
  --runtime-version 1.21.2 \
