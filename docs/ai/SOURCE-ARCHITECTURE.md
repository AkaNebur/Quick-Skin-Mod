...
- `common/src/test`: loader-independent JUnit regression tests compiled against the common 26.1
  node.
...
|---|---|---|
| common | 26.1 | `common/src/legacy26_1` |
| fabric | 26.1 | none; canonical output |
| neoforge | 26.1 | none; canonical output |

The common overlay owns the single immediate-extraction preview backend selected on 26.1 and
excludes the canonical deferred-collector backend that requires the newer GUI API. All other 26.1 API deltas are
