...
- `common/src/test`: loader-independent JUnit regression tests compiled against the common 1.21.2
  node.
...
|---|---|---|
| common | 1.21.2 | `common/src/legacy1_21_2` |
| fabric | 1.21.2 | none; canonical output |
| neoforge | 1.21.2 | `neoforge/src/legacy1_21_2` |

The NeoForge whole-file replacements are genuine 1.21.2 rewrites of `CapeLayerMixin`,
`PlayerRendererMixin`, `PlayerInfoMixin`, `MixinAbstractClientPlayer`, and the thin NeoForge 21.2
`PlatformHelperImpl` loader adapter. `SkinManagerMixin` deliberately remains canonical so the
1.21.2 lane retains its `HttpTexture` bridge and `CompletableFuture<PlayerSkin>` contract.
Common overlay Java files are additive compatibility classes, the exact 1.21.2 cape render-state
