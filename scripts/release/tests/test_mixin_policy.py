...
# renderHand requests two buffers through 1.21.1, one buffer from 1.21.2 through 1.21.10, and the
# later renderer submits one model part. SkinManager 1.20.1, 1.21.1, and 1.21.2-1.21.5 each have two
...
class MixinPolicyTest(unittest.TestCase):
    def mixin_sources(self) -> tuple[Path, ...]:
        sources: list[Path] = []
        for source_root in (CANONICAL_JAVA, *OVERLAY_JAVA):
    ...
    def test_active_loader_mixins_have_explicit_failure_and_count_policies(self) -> None:
        for artifact in MATRIX["artifacts"]:
            loader = artifact["loader"]
            version = artifact["artifact_version"]
            overlay_name = MATRIX["source_overlays"].get(loader, {}).get(version)
            canonical_root = ROOT / loader / "src" / "main"
            overlay_root = ROOT / loader / "src" / overlay_name if overlay_name else None
            config_name = f"quickskin-{loader}.mixins.json"
            config_path = (
                overlay_root / "resources" / config_name
                if overlay_root is not None
                and (overlay_root / "resources" / config_name).is_file()
                else canonical_root / "resources" / config_name
            )
            if not config_path.is_file():
                continue

            config = json.loads(config_path.read_text(encoding="utf-8"))
            with self.subTest(config=relative(config_path)):
                self.assertIs(config["required"], True)
                self.assertEqual(config["injectors"]["defaultRequire"], 1)

            package_path = Path(*config["package"].split("."))
            configured = tuple(config.get("client", ())) + tuple(config.get("mixins", ()))
            for class_name in configured:
                relative_source = package_path / Path(*class_name.split(".")).with_suffix(".java")
                candidates = []
                if overlay_root is not None:
                    candidates.append(overlay_root / "java" / relative_source)
                candidates.append(canonical_root / "java" / relative_source)
                source_path = next((path for path in candidates if path.is_file()), None)
                self.assertIsNotNone(
                    source_path,
                    f"{relative(config_path)} configures missing mixin {class_name}",
                )
                assert source_path is not None
                text = source_path.read_text(encoding="utf-8")
                annotations = list(INJECTOR_MARKER.finditer(text))
                self.assertTrue(annotations, relative(source_path))
                for annotation in annotations:
                    handler = HANDLER_MARKER.search(text, annotation.end())
                    self.assertIsNotNone(handler, f"{relative(source_path)}: injector has no handler")
                    assert handler is not None
                    handler_name = handler.group(1)
                    context = text[annotation.start() : handler.start()]
                    expected_require = (
