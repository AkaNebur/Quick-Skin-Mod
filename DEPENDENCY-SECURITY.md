...
- `gradle/verification-metadata.xml` verifies both artifacts and Maven/Gradle metadata with
  ...
  Minecraft, mappings, transform, runtime, native, and E2E classpaths for the active 1.21.1 graph.
...
./gradlew --no-daemon --no-parallel \
  ...
  :common:1.21.1:dependencies \
  :fabric:1.21.1:dependencies \
  :neoforge:1.21.1:dependencies
```
